...
struct sub_request * find_read_sub_request(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die)
{
	...
	struct sub_request *sub=NULL,* p=NULL,*update_sub_write=NULL;
	...
	for(plane=0;plane<ssd->parameter->plane_die;plane++)
	{
		...
		if(address_ppn!=-1)
		{
			...
			{
				/**********************************Refresh*****************************************/
				ssd->access_tail_latency[ssd->dram->map->map_entry[sub->lpn].pagetype]++;
...
int delete_w_sub_request(struct ssd_info * ssd, unsigned int channel, struct sub_request * sub )
{
	...
	ssd->dram->map->map_entry[sub->lpn].pagetype=1;//by lyn
	ssd->dram->map->map_entry[sub->lpn].readnum=0;//by lyn ,处理完后相当于更新，读次数设置为0
...
Status copy_back(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die,struct sub_request * sub)
{
	...
	if (ssd->parameter->greed_CB_ad==1)                                               /*允许贪婪使用copyback高级命令*/
	{
		...
		if (ssd->dram->map->map_entry[sub->lpn].state!=0)                             /*说明这个逻辑页之前有写过，需要使用copyback+random input命令，否则直接写下去即可*/
		{
			...
			{
				//write by lyn
				sub->next_state_predict_time=ssd->current_time+19*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub->lpn].pagetype]+(sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
...
//by lyn
				...
				{
						sub->next_state_predict_time=ssd->current_time+7*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub->lpn].pagetype]+(size(ssd->dram->map->map_entry[sub->lpn].state))*ssd->parameter->time_characteristics.tRC+(sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
				}
				ssd->read_count++;
				ssd->update_read_count++;
			}
...
struct ssd_info *delete_from_channel(struct ssd_info *ssd,unsigned int channel,struct sub_request * sub_req)
{
	...
	*******************************************************************/
	...
	while (sub!=NULL)
	{
		if (sub==sub_req)
		{
			ssd->dram->map->map_entry[sub_req->lpn].readnum=0;//by lyn ,处理完后相当于更新，读次数设置为0
			ssd->dram->map->map_entry[sub_req->lpn].pagetype=1; //by lyn
			if ((ssd->parameter->allocation_scheme==0)&&(ssd->parameter->dynamic_allocation==0))
			{
...
struct ssd_info *un_greed_interleave_copyback(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,struct sub_request *sub1,struct sub_request *sub2)
{
	...
	if ((old_ppn1%2==ppn1%2)&&(old_ppn2%2==ppn2%2))
	{
		...
		sub1->next_state_predict_time=ssd->current_time+14*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub1->lpn].pagetype]+(sub1->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		...
		sub2->next_state_predict_time=sub2->current_time+14*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub2->lpn].pagetype]+(sub2->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
	...
	else if ((old_ppn1%2!=ppn1%2)&&(old_ppn2%2==ppn2%2))
	{
		...
		sub2->next_state_predict_time=ssd->current_time+14*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub2->lpn].pagetype]+(sub2->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
	...
	else
	{
		...
		sub1->next_state_predict_time=ssd->current_time+14*ssd->parameter->time_characteristics.tWC+ssd->tail_value[ssd->dram->map->map_entry[sub1->lpn].pagetype]+2*(ssd->parameter->subpage_page*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		...
		{	
			...
			{   
			    ...
			    ******************************************************************************************************/
				...
				sub->next_state_predict_time=ssd->current_time+ssd->tail_value[ssd->dram->map->map_entry[sub->lpn].pagetype];
			...
			case SR_W_TRANSFER:
			{
				...
				*******************************************************************************************************/
				...
				printf("sub: %10lld  %10lld %10lld \n", sub->begin_time,sub->complete_time,time+ssd->parameter->time_characteristics.tPROG);
