...
int  main(int argc, char const *argv[])
{
	unsigned  int i,j,k;
	struct ssd_info *ssd;
	char outputfilename[200],statisticfilename[200],tmp1[40],tmp2[40];
	char filename[16][40]={{"STG_0"},{"HM_1"},{"USR_0"},{"SRC1_2"},{"SRC2_0"},{"SRC2_2"},{"PROJ_3"},{"PROJ_4"},{"PROJ_1"},{"PRN_0"},{"PRN_1"},{"PROJ_0"},{"HM_0"},{"PRXY_0"},{"RSRCH"},{"WDEV_0"}};//by lyn
	#ifdef DEBUG
		printf("enter main\n"); 
	#endif
	
	unsigned int filei;
	for (filei = 12;filei < 13; filei++)// by lyn trace file number
	{
		printf("start simulate %s\n",filename[filei]);
		memset(outputfilename,0,sizeof(outputfilename));
		memset(statisticfilename,0,sizeof(statisticfilename));
		...
		{
			printf("%s\n", "running as default");
			strcpy(ssd->tracefilename,filename[filei]);//HM_0,HM_1,PRXY_0,PRN_0/1/PROJ_1(too much free page),PROJ_3,PROJ_4,PRXY_0,RSRCH,SRC1_2,SRC2_0,SRC2_2(error there is no free page),STG_0,USR_0,WDEV_0

			strcpy(tmp1,filename[filei]);
			strcpy(tmp2,filename[filei]);
			strcat(tmp1,"_output");
			strcpy(ssd->outputfilename,tmp1);
			strcat(tmp2,"_statistic");
			strcpy(ssd->statisticfilename,tmp2);
		...
		ssd->current_file=filei;
		ssd=simulate(ssd);
		
		statistic_output(ssd);  
		/*	free_all_node(ssd);*/
		//printf("sub complete time=%d\n",ssd->ttmp);
		printf("\n");
...
struct ssd_info *simulate(struct ssd_info *ssd)
{
	...
	while(flag!=100)      
	{
		...
		if(flag == 0 && ssd->request_queue == NULL)
			flag = 100;
		//if(ssd->read_request_count+ssd->write_request_count >= 999098)
			//flag =100;
	}
...
{  
	...
	***********************************************************************************************************/
	...
	if (nearest_event_time==MAX_INT64)
	{
		...
		//if (ssd->request_queue_length>ssd->parameter->queue_length)    //脠莽鹿没脟毛脟贸露脫脕脨碌脛鲁鈧睹埪陈姑矫伱嬅吤っ柮兠幟勁捗久柮惷嬅姑壝柮兟得劼斥偓露脠                     
		//{
...
void trace_output(struct ssd_info* ssd){
	int flag = 1,i;	
	int64_t start_time, end_time, reqtmp;
	struct request *req, *pre_node;
	struct sub_request *sub, *tmp,*tmplyn=NULL,*update_sub_write=NULL;
	...
	while(req != NULL)	
	{
		...
		if(req->response_time != 0)
		{
			...
			if (req->operation==READ)
			{
				...
				if(ssd->max_read_latency<(req->response_time-req->time))
					ssd->max_read_latency=(req->response_time-req->time);
				
				for(i=1;i<area;i++){// by lyn,for count read latency distribute
					if((req->response_time-req->time)>=ssd->latency_num[i-1]&&(req->response_time-req->time)<ssd->latency_num[i]){
						ssd->latency_count[i-1]++;
						break;
					}
				}
				if(i==area)
					...
					/*****************************************优先队列统计尾端延迟********************************************/
							...
							/*	if (tail.size()<q_capacity){
									tail.push(req);
								}
								else
								{
									if ((req->response_time-req->time)>(tail.head()->response_time-tail.head()->time)){
										tail.pop();
										tail.push(req);
									}
								}*/
				
								reqtmp=(req->response_time)-(req->time);
								if(reqtmp>tail_threshold){
									if(ssd->tail_max->tail_max_size<ssd->q_capacity[ssd->current_file]){			//by lyn, 队列未满时，需要注意的是tail_max_size最大只可能是q_capacity
										ssd->tail_max->tail_heap[ssd->tail_max->tail_max_size]=reqtmp;
										ssd->tail_max->tail_max_size++;
										HeapSort(ssd->tail_max->tail_heap,ssd->tail_max->tail_max_size);
									}
									else{
										if(ssd->tail_max->tail_heap[0]<reqtmp)
										{
											ssd->tail_max->tail_heap[0]=reqtmp; 			// 首先，把队列中最小延迟的请求删掉，也就是头部第一个,然后插入一个请求，与上述不同的是不用对size变量加一
											HeapSort(ssd->tail_max->tail_heap,ssd->tail_max->tail_max_size);
										}
									}
								}
		...
		else
		{
			...
			{		ssd->ttmp++;
				...
				if (req->operation==READ)
				{
					...
					if(ssd->max_read_latency<(end_time-req->time))
					ssd->max_read_latency=(end_time-req->time);
				
					for(i=1;i<area;i++){// by lyn,for count read latency distribute
					if((end_time-req->time)>=ssd->latency_num[i-1]&&(end_time-req->time)<ssd->latency_num[i]){
						ssd->latency_count[i-1]++;
						break;
						}
					}
...
{
	unsigned int cdf[area+1];
	int64_t cdf_l[area+1],sm;
	...
	fprintf(ssd->statisticfile,"refresh count:\t %13d\n",ssd->refresh_count);		// by lyn,refresh count 

//	for(i=1;i<area;i++)//by lyn
	//	fprintf(ssd->statisticfile,"[%d,%d):\t %13d\n",ssd->latency_num[i-1],ssd->latency_num[i],ssd->latency_count[i-1]);
	//fprintf(ssd->statisticfile,"[%d,%d]:\t %13d\n",ssd->latency_num[i-1],ssd->max_read_latency,ssd->latency_count[i-1]);
	...
	memset(cdf,0,sizeof(cdf));
		...
		fprintf(ssd->statisticfile,"tail_max_size:\t %13d\n",ssd->tail_max->tail_max_size);		// by lyn
	
		for (i = 0; i < ssd->tail_max->tail_max_size; i++){ 	//by lyn,print every latency
		
			//fprintf(ssd->statisticfile,"latency %d :\t %lld\n",i+1,ssd->tail_max->tail_heap[i]);
	
			for(j=1;j<area;j++){// by lyn,for count read latency distribute
				if(ssd->tail_max->tail_heap[i]>=cdf_l[j-1]&&ssd->tail_max->tail_heap[i]<cdf_l[j]){
					cdf[j-1]++;
					break;
				}
			}
		...
		sm=ssd->read_request_count-ssd->tail_max->tail_max_size;//ssd->pct_last_latency[0]=0;
	ssd->pct_last_latency[0]=(sm*1.0)/ssd->read_request_count;
	for (i = 1; i <= area; i++){
		fprintf(ssd->statisticfile,"cdf[%d,%d):\t %13d\n",cdf_l[i-1],cdf_l[i],cdf[i-1]);

		sm+=cdf[i-1];
		ssd->pct_last_latency[i]=(sm*1.0)/ssd->read_request_count;
	}
	fprintf(ssd->statisticfile,"pctstart:\t %.6f\n",ssd->pct_last_latency[0]*100);
	for(i=1;i<=area;i++)
		fprintf(ssd->statisticfile,"pct[%d,%d):\t %.6f\n",cdf_l[i-1],cdf_l[i],ssd->pct_last_latency[i]*100);

	for(i=1;i<8;i++){
		fprintf(ssd->statisticfile,"access tail latency %13d count:\t %13d\n",i*80,ssd->access_tail_latency[i]);
	}
...
void PercDown(int64_t   A[], int64_t i, int64_t N)
{
    int64_t child;
    int64_t Tmp;
