...
#define area 200
#define refresh_latency 400000


#define tail_threshold 100000		//by lyn, large than tail_threshold, and collect


#define q_capacity_max 50000
...
struct ac_time_characteristics{
	...
	int tRinit;		//by elainelv, the initialized read latency
...
struct ssd_info{ 
	...
	unsigned int refresh_count;			 // by lyn, using to record the refresh number
	unsigned int latency_num[area+1];
	unsigned int latency_count[area+1];
	unsigned int max_read_latency;
	
	struct Tail_l *tail_max;
	double pct_last_latency[area+1]; //by lyn, to compute percentle

	unsigned int q_capacity[20];				//by lyn, how many slowest request should collect
	unsigned int current_file;

	unsigned int ttmp;// by lyn

	unsigned int access_tail_latency[10];
	unsigned int tail_value[10];// for 7 levels
...
struct Tail_l{ // by lyn
	unsigned int tail_max_size;
	int64_t tail_heap[q_capacity_max];
...
struct entry{                       
	...
	unsigned int readnum;			//by lyn, 表示当前逻辑地址对应物理地址上数据在上一次更新之后被读取的次数;根据读取次数的值，超过设定的阈值时，再一次进行刷新写回
	unsigned int pagetype;			//by lyn, 表示页面的类型，用于区分数据存放的位置
};
